...
import { prisma } from "@/lib/prisma"; // Assuming you use the singleton instance
...
export default async function ClientDashboard() {
  ...
  if (!userId) return <div>{t("unauthorized")}</div>;
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-10 bg-background text-foreground animate-in fade-in duration-500">
        ...
        <Link href="/client/rewards">
          ...
          >
            ...
            {activeTickets.length > 0 && (
              // Changed 'right-0' to 'end-0' for RTL support (Arabic)
              <span className="absolute top-0 end-0 h-2.5 w-2.5 rounded-full bg-primary ring-2 ring-background" />
            )}
          </Button>
        </Link>
      </div>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-[#0B0C0E] border border-white/10 shadow-2xl mb-12 group transition-transform active:scale-[0.98]">
        {/* Background Gradients: Changed 'right-0'/'left-0' to 'end-0'/'start-0' */}
        <div className="absolute top-0 end-0 w-64 h-64 bg-primary/20 blur-[80px] rounded-full mix-blend-screen opacity-50" />
        <div className="absolute bottom-0 start-0 w-40 h-40 bg-indigo-900/20 blur-[60px] rounded-full opacity-50" />
      ...
      {activeTickets.length > 0 && (
        <div className="mb-10 space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            >
              {t("view_all")} {/* rtl:rotate-180 flips the arrow for Arabic */}
